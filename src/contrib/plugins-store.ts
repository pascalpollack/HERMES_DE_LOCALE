...
export interface PluginRecord {
  ...
  description?: string
  /** Dot-paths into the plugin's OWN locale bundles for name and one-liner
   *  (see `HermesPlugin.nameKey`). The record holds keys rather than resolved
   *  text because it is built before the plugin registers its bundles; the
   *  Plugins page resolves them at render time and falls back to `name` /
   *  `description`. */
  nameKey?: string
