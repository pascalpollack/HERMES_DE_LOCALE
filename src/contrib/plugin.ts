...
export interface HermesPlugin {
  ...
  id: string
  /** Human name for settings / about UI. English, and the FALLBACK — see
   *  `nameKey`. */
  name?: string
  /** One-liner for the settings inventory (what the plugin adds). English,
   *  and the fallback — see `descriptionKey`. */
  description?: string
  /** Dot-path into THIS plugin's own locale bundles for the display name. The
   *  inventory record carries the KEY, not the resolved string: the record is
   *  built before `register()` runs, which is where the bundles are declared,
   *  and a string resolved at load time would freeze the language of that
   *  moment. The Plugins page resolves it while rendering (reactive through
   *  `usePluginI18n`), and falls back to `name` whenever the key does not
   *  resolve — a disabled plugin has torn its bundles down, so the fallback is
   *  the normal case, not the exception. */
  nameKey?: string
  /** Same contract as `nameKey`, for the one-liner. */
