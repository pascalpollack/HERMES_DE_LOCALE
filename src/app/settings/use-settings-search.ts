...
export function useSettingsSearchCatalog(enabled: boolean) {
  ...
  const pluginEntries: PluginSearchEntry[] = [
    ...Object.values(desktopPluginRecords).map(record => ({
      ...
      description: resolvePluginLabel(translatePluginKey, record.id, record.descriptionKey, record.description ?? ''),
      icon: Monitor,
      id: `plugin:desktop:${record.id}`,
      // The ENGLISH name stays a keyword next to the id: someone who knows the
      // plugin as "Bots" finds it in a German UI too.
      keywords: ['plugin', 'extension', 'desktop', record.id, record.name],
      label: resolvePluginLabel(translatePluginKey, record.id, record.nameKey, record.name),
      plugin: record.id
    })),
