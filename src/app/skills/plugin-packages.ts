...
export interface PluginPackage {
  ...
  name: string
  description: string
  /** The DESKTOP half's plugin id and its own translation keys, when it has
   *  them — the Plugins page resolves them at render time (see
   *  `resolvePluginLabel`). `name`/`description` above stay the English
   *  fallback. An agent-only package has no desktop half and none of these. */
  pluginId?: string
  nameKey?: string
  ...
  for (const record of desktopRecords) {
    ...
    if (pkg) {
      ...
      if (existing) {
        ...
        existing.descriptionKey = record.descriptionKey
