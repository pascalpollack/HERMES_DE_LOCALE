...
registry.registerMany([
  {
    id: 'sessions',
    area: 'panes',
    // `title` stays the ENGLISH IDENTIFIER — the zone menu, the ⌘K toggles and
    // the drag payload key off it. What the TAB shows is `data.tabTitle`: a
    // function, called on every render, so a language switch lands without a
    // restart (`translateNow` itself is not reactive).
    title: 'sessions',
    ...
    data: {
      tabTitle: () => translateNow('zones.paneTitles.sessions'),
      placement: 'left',
      collapsible: true,
  ...
  },
  {
    ...
    data: {
      tabTitle: () => translateNow('zones.paneTitles.terminal'),
      placement: 'bottom',
      height: '20vh',
      maxHeight: '80vh',
      revealOnPreset: true,
  ...
  },
  {
    ...
    data: {
      tabTitle: () => translateNow('zones.paneTitles.files'),
      placement: 'right',
      collapsible: true,
  ...
  },
  {
    ...
    data: {
      tabTitle: () => translateNow('zones.paneTitles.review'),
      placement: 'right',
      collapsible: true,
...
const syncLogsPane = (open: boolean) => {
  if (open) {
    unregisterLogsPane ??= registry.register({
      ...
      data: {
        tabTitle: () => translateNow('zones.paneTitles.logs'),
        placement: 'bottom',
