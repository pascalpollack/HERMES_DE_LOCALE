...
export interface Translations {
  ...
  zones: {
    /** TAB LABELS of the standing panes. Separate from the pane `title` in
     *  `contrib/controller.tsx`, which stays the ENGLISH IDENTIFIER — these are
     *  rendered through `PaneChrome.tabTitle`, so a language switch takes
     *  effect on the next render instead of on the next start. */
    paneTitles: {
      sessions: string
      terminal: string
      files: string
