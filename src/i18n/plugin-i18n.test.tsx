...
describe('plugin display label', () => {
  const translate = (pluginId: string, key: string, ...args: unknown[]) =>
    translatePlugin(pluginId, 'de', key, args)

  it('prefers the translation a plugin ships for itself', () => {
    const dispose = registerPluginLocales('bots', {
      en: { plugin: { name: 'Bots' } },
      de: { plugin: { name: 'Botten' } }
    })

    expect(resolvePluginLabel(translate, 'bots', 'plugin.name', 'Bots')).toBe('Botten')

    dispose()
  })

  it('falls back to the English manifest text when the bundles are gone', () => {
    // Exactly what a DISABLED plugin looks like: the loader disposed its
    // bundles, so the key no longer resolves. The row must still read "Bots"
    // and never the raw dot-path.
    const dispose = registerPluginLocales('bots', { de: { plugin: { name: 'Botten' } } })
    dispose()

    expect(resolvePluginLabel(translate, 'bots', 'plugin.name', 'Bots')).toBe('Bots')
  })

  it('falls back for a plugin that ships no keys at all', () => {
    expect(resolvePluginLabel(translate, 'third-party', undefined, 'Pixel Overlay')).toBe('Pixel Overlay')
  })
})
