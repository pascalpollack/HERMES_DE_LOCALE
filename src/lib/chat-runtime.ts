 ...
 *
 *  A FUNCTION, not a constant: the placeholder is shown, so it is translated,
 *  and a module-level constant would freeze the language of the first import.
 *  Callers that render it inside React take `t.zones.paneTitles.newSession`
 *  from `useI18n()` instead — that re-renders on a language switch. */
export function newSessionTitle(): string {
  return translateNow('zones.paneTitles.newSession')
}
