...
export default {
  id: ID,
  // name/description stay ENGLISH here on purpose: the inventory record is
  // built BEFORE register() runs, which is where the bundles are declared, so
  // this is the fallback the Plugins page shows until they are loaded — and
  // again after a disable tears them down. nameKey/descriptionKey point into
  // this plugin's own bundles and are resolved while rendering.
  name: 'Bots',
  nameKey: 'plugin.name',
  ...
  register(ctx: PluginContext) {
    ...
    ctx.register({
      ...
      data: {
        // `title` above stays the English identifier (zone menu, drag key);
        // the TAB reads this, re-evaluated on every render, so a language
        // switch lands without a restart.
        tabTitle: () => ctx.i18n.t('pane.title'),
        placement: 'left',
        width: '260px',
        collapsible: true,
        hideOnly: true,
