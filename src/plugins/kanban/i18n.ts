...
export const en: KanbanMessages = {
  plugin: {
    name: 'Kanban',
    description:
      'Multi-agent task board — board page, sidebar entry, and a live in-flight count in the status bar.'
  },
  nav: 'Kanban',
...
const ja: KanbanMessages = {
  plugin: {
    name: 'カンバン',
    description:
      '複数エージェント向けのタスクボード — ボード画面、サイドバーの項目、そしてステータスバーの実行中件数。'
  },
  nav: 'カンバン',
...
const zh: KanbanMessages = {
  plugin: {
    name: '看板',
    description:
      '多智能体任务看板 — 看板页面、侧边栏入口，以及状态栏中的实时进行数。'
  },
  nav: '看板',
  openBoard: '看板：打开面板',
...
const zhHant: KanbanMessages = {
  plugin: {
    name: '看板',
    description:
      '多智能體工作看板 — 看板頁面、側邊欄入口，以及狀態列中的即時進行數。'
  },
  nav: '看板',
  openBoard: '看板：開啟面板',
...
const de: KanbanMessages = {
  plugin: {
    name: 'Kanban',
    description:
      'Aufgabentafel für mehrere Agenten — eigene Seite, Eintrag in der Seitenleiste und die Zahl der laufenden Aufgaben in der Statuszeile.'
  },
  nav: 'Kanban',
  openBoard: 'Kanban: Board öffnen',
  newTaskCommand: 'Kanban: Neuer Task',
  countTip: (running, ready) => `Kanban, ${running} laufend, ${ready} bereit`,
  col: {
    triage: { label: 'Triage', help: 'Rohe Ideen, ein Specifier arbeitet die Spezifikation aus.' },
    todo: { label: 'Offen', help: 'Wartet auf Abhängigkeiten oder ist nicht zugewiesen.' },
    scheduled: { label: 'Geplant', help: 'Wartet auf einen geplanten Zeitpunkt.' },
    ready: { label: 'Bereit', help: 'Abhängigkeiten erfüllt, ein Profil zuweisen und der Dispatcher führt es aus.' },
    ...
    done: { label: 'Erledigt', help: 'Abgeschlossen, abhängige Kinder werden bereit.' },
    archived: { label: 'Archiviert', help: 'In der Standardansicht des Boards ausgeblendet.' }
  },
  locked: {
    ...
    scheduled: 'Geplant braucht einen Weckzeitpunkt, Agenten setzen ihn, per Drag geht es nicht.'
  },
  ...
  noMatch: 'Keine Tasks passen zu den Filtern',
  noTasks: 'Keine Tasks auf diesem Board',
  open: 'Öffnen',
  ...
  attachedTip: name => `${name} ist zugewiesen, der Dispatcher übergibt beim nächsten Tick (≤1 Min).`,
  orchestratorTip: name => `${name} (der Orchestrator) übernimmt beim nächsten Tick und schreibt die Spezifikation.`,
  autoAssignTip: name => `Wird beim nächsten Dispatch-Tick automatisch "${name}" (kanban.default_assignee) zugewiesen.`,
  wontRun: "läuft nicht",
  wontRunTip:
    'Karten in Bereit laufen erst, wenn ein Profil zugewiesen ist. Karte öffnen und einen Zuständigen setzen, oder in den Orchestrierungseinstellungen einen Standard-Zuständigen festlegen.',
  noHeartbeat: 'kein Heartbeat',
  expand: label => `${label} ausklappen`,
  collapse: label => `${label} einklappen`,
  newTaskIn: label => `Neuer Task in ${label}`,
  empty: 'Leer',
  ...
  titlePlaceholderTriage: 'Grobe Idee, ein Specifier arbeitet sie aus',
  ...
  modelHint: 'Führt diesen Task mit einem bestimmten Modell und einer bestimmten Denktiefe aus. Nicht gesetzt verwendet die Einstellung des zugewiesenen Profils.',
  workspaceInherit: 'Übernimmt das Projektverzeichnis des Boards',
  workspaceInheritDir: dir => `Leer lassen, um ${dir} zu übernehmen`,
  workspaceInheritGeneric: 'Leer lassen, um das Projektverzeichnis des Boards zu übernehmen.',
  assignee: 'Zuständig',
  defaultOption: name => `${name} (Standard)`,
  parkedOption: "nicht zugewiesen (geparkt, läuft nicht)",
  skills: 'Skills (kommagetrennt)',
  skillsPlaceholder: 'translation, github',
  parent: "Übergeordnet (blockiert, bis dieser fertig ist)",
  noParent: '— kein Übergeordneter —',
  goalMode: "Zielmodus (Worker läuft in Schleife, bis ein Judge zustimmt, dass es fertig ist)",
  ...
  makesModelCall: 'macht einen Modellaufruf',
  estimateTip: 'Grobe Token- und Komplexitätsschätzung vom Hilfsmodell, macht einen Modellaufruf.',
  estimateTipLong: 'Führt einen kurzen Hilfsmodell-Aufruf aus, um Token und Komplexität zu schätzen. Ein grober Richtwert, keine Rechnung.',
  roughEstimate: 'Grobe Schätzung',
  tokUnit: 'Tok',
  couldNotEstimate: 'Konnte nicht geschätzt werden',
  complexity: { S: 'Klein', M: 'Mittel', L: 'Groß' },
  introBody:
    'Du führst die Karten nicht selbst aus, Agenten tun das. Lege eine Karte mit Zuständigem in Bereit, ein Agent übernimmt sie innerhalb einer Minute. Ohne Zuständigen läuft nichts. Triage: ein Agent schreibt die Idee zuerst zu einem richtigen Task um. Offen: wartet auf andere Karten. Geplant: wartet auf einen Timer. Läuft und Review: die Spuren der Agenten, nicht anfassen. Blockiert: wartet auf dich. Ergebnisse kommen zurück auf die Karte.',
  introGotIt: 'Verstanden',
  evtCreated: (where, assignee) =>
    `erstellt${where ? ` in ${where}` : ''}${assignee ? ` · zugewiesen an ${assignee}` : ''}`,
  evtMovedTo: col => `verschoben nach ${col}`,
  evtParentReopened: parent => `übergeordneter Task ${parent} wieder geöffnet`,
  evtAssignedTo: assignee => `zugewiesen an ${assignee}`,
  ...
  evtUnblocked: col => `Blockierung aufgehoben${col ? ` → ${col}` : ' → Bereit'}`,
  evtReclaimed: 'zurückgeholt, in die Warteschlange zurückgestellt',
  evtSpecified: 'Spezifikation vom Triage-Agenten geschrieben',
  ...
  metaWorkerPid: 'Worker-PID',
  readyUnassignedTitle: 'Bereit, aber nicht zugewiesen, diese Karte läuft nie.',
  readyUnassignedBody:
    'Der Dispatcher übernimmt nur Karten in Bereit, die einen Zuständigen haben. Wähle oben im Feld Zuständig ein Profil (oder lege in den Orchestrierungseinstellungen einen Standard-Zuständigen fest), dann läuft sie innerhalb einer Minute.',
  ...
  commentsHelpRunning:
    'Dieser Task läuft gerade. Deine Notiz wird innerhalb weniger Sekunden in den aktuellen Turn des Workers eingefügt, kein Blockieren/Entsperren nötig. "Mit Notiz neu einreihen" startet den Task stattdessen von vorn, mit deiner Notiz im Kontext.',
  commentsHelp:
    'Kommentare werden zum Task-Thread hinzugefügt. Wenn ein Worker den Task übernimmt, liest er sie als Teil seines Kontexts.',
  send: 'Senden',
  comment: 'Kommentar',
  messageWorker: 'Dem laufenden Worker eine Nachricht schreiben…',
  addComment: 'Kommentar hinzufügen…',
  deliveredLive: 'Wird innerhalb weniger Sekunden an den laufenden Worker zugestellt.',
  requeueWithNote: 'Mit Notiz neu einreihen',
  notePosted: 'Notiz gepostet, Worker neu eingereiht',
  ...
  boardImportedAs: slug => `Der Name war schon vergeben, importiert als ${slug}`,
  renameBoardTitle: 'Board umbenennen',
  deleteBoardTitle: name => `"${name}" löschen?`,
  deleteBoardConfirm: 'Das Board wird archiviert, nicht gelöscht, seine Tasks und Anhänge bleiben auf der Festplatte und können wiederhergestellt werden.',
  ...
  projectHintPre:
    'Neue Tasks laufen im Repo des Projekts (ein Worktree pro Task), jeder Task kann seinen Arbeitsbereich bei der Erstellung trotzdem überschreiben. Projekte verwalten mit ',
  projectHintCmd: 'hermes project',
  createBoard: 'Board erstellen',
  orchestratorProfile: 'Orchestrator-Profil',
  defaultAssignee: 'Standard-Zuständiger',
  defaultParen: '(Standard)',
  autoDecompose: 'Triage-Tasks automatisch zerlegen',
  profileDescriptions: 'Profilbeschreibungen',
  profileDescriptionsHint:
    'Beschreibungen leiten das Routing des Decomposers. Automatisch mit dem Hilfsmodell erzeugen oder selbst schreiben.',
  profileGoodAt: 'Worin ist dieses Profil gut?',
  auto: 'Auto',
  notify: {
    completedTitle: 'Task abgeschlossen',
    blockedTitle: 'Task blockiert, braucht deine Eingabe',
    blockLoopTitle: 'Task an Triage weitergeleitet, braucht eine Entscheidung',
    gaveUpTitle: 'Task aufgegeben',
    crashedTitle: 'Worker abgestürzt, wird erneut versucht',
    timedOutTitle: 'Task hat Zeitüberschreitung, wird erneut versucht',
...
const ru: KanbanMessages = {
  plugin: {
    name: 'Канбан',
    description:
      'Доска задач для нескольких агентов — страница доски, пункт в боковой панели и число активных задач в строке состояния.'
  },
  nav: 'Канбан',
  ...
  col: {
    triage: { label: 'Сортировка', help: 'Сырые идеи — специфаер доводит их до полноценной задачи.' },
    todo: { label: 'К выполнению', help: 'Ожидает зависимостей или не назначено.' },
    scheduled: { label: 'По расписанию', help: 'Ожидает наступления запланированного времени.' },
    ready: { label: 'Готово', help: 'Зависимости выполнены — назначьте профиль, и диспетчер запустит задачу.' },
    running: { label: 'Выполняется', help: 'Взято воркером — над задачей работает агент. Устанавливается диспетчером.' },
    blocked: { label: 'Заблокировано', help: 'Воркер запросил участие человека.' },
    review: { label: 'Проверка', help: 'Агент-проверяющий проверяет работу. Устанавливается диспетчером.' },
    done: { label: 'Готово', help: 'Завершено; зависимые дочерние задачи становятся готовыми.' },
    archived: { label: 'В архиве', help: 'Скрыто из стандартного вида доски.' }
  },
  locked: {
    review: 'Статус «Проверка» устанавливается диспетчером, когда карточку берёт агент-проверяющий.',
    running: 'Статус «Выполняется» устанавливается диспетчером, когда карточку берёт воркер.',
    scheduled: 'Для статуса «По расписанию» нужно время пробуждения — его задают агенты; перетащить карточку сюда нельзя.'
  },
  arcRunning: 'Сейчас над этим работает агент.',
  arcStale: 'Взято в работу, но от воркера нет сигнала уже 2+ минуты — диспетчер заберёт задачу обратно.',
  title: 'Канбан',
  orchestrationSettings: 'Настройки оркестрации',
  newTask: 'Новая задача',
  filterCards: 'Фильтр карточек…',
  noMatch: 'Нет задач, соответствующих фильтрам',
  ...
  delete: 'Удалить',
  reviewChecking: 'Агент-проверяющий проверяет выполненную работу.',
  attachedTip: name => `${name} назначен — диспетчер передаст задачу на следующем такте (≤1 мин).`,
  orchestratorTip: name => `${name} (оркестратор) заберёт задачу на следующем такте и напишет спецификацию.`,
  autoAssignTip: name => `Будет автоматически назначено на «${name}» (kanban.default_assignee) на следующем такте диспетчера.`,
  wontRun: 'не будет выполнено',
  wontRunTip:
    'Карточки в статусе «Готово» запускаются только после назначения профиля. Откройте карточку и укажите исполнителя, либо задайте исполнителя по умолчанию в настройках оркестрации.',
  noHeartbeat: 'нет сигнала',
  ...
  groupRunning: 'Группировать «Выполняется» по профилю',
  nSelected: n => `Выбрано: ${n}`,
  moveToShort: 'Переместить в',
  assign: 'Назначить',
  unassignAction: 'Снять назначение',
  archive: 'Архивировать',
  clearSelection: 'Очистить выбор (Esc)',
  refused: 'отклонено',
  bulkFailed: (failed, total, err) => `Не выполнено ${failed} из ${total} — ${err}. Карточки с ошибкой остаются выбранными.`,
  titlePlaceholderTriage: 'Черновая идея — специфаер доработает её',
  titlePlaceholder: 'Название',
  descPlaceholder: 'Описание (необязательно)',
  priority: 'Приоритет',
  workspace: 'Рабочее пространство',
  boardDefaultSuffix: ' · по умолчанию для доски',
  workspaceOverride: 'Путь к рабочему пространству (необязательное переопределение)',
  model: 'Модель',
  modelInherit: 'По умолчанию из профиля',
  modelClear: 'Сбросить переопределение модели',
  modelHint: 'Запускает эту задачу на конкретной модели и глубине рассуждений. Если не задано, используется значение назначенного профиля.',
  workspaceInherit: 'Наследует каталог проекта доски',
  workspaceInheritDir: dir => `Оставьте пустым, чтобы унаследовать ${dir}`,
  workspaceInheritGeneric: 'Оставьте пустым, чтобы унаследовать каталог проекта доски.',
  assignee: 'Исполнитель',
  defaultOption: name => `${name} (по умолчанию)`,
  parkedOption: 'не назначено (отложено — не будет выполнено)',
  skills: 'Навыки (через запятую)',
  skillsPlaceholder: 'translation, github',
  parent: 'Родительская задача (блокирует до её завершения)',
  noParent: '— нет родительской задачи —',
  goalMode: 'Режим цели (воркер повторяет цикл, пока судья не признает задачу выполненной)',
  creating: 'Создание…',
  createTask: 'Создать задачу',
  cancel: 'Отмена',
  save: 'Сохранить',
  estimate: 'Оценка',
  estimateEffort: 'Оценить трудозатраты',
  estimating: 'Оценка…',
  reEstimate: 'Оценить заново',
  makesModelCall: 'делает вызов модели',
  estimateTip: 'Приблизительная оценка токенов и сложности от вспомогательной модели — выполняет вызов модели.',
  estimateTipLong: 'Выполняет быстрый вызов вспомогательной модели для оценки токенов и сложности. Ориентировочная оценка, а не счёт.',
  roughEstimate: 'Приблизительная оценка',
  tokUnit: 'ток.',
  couldNotEstimate: 'Не удалось оценить',
  complexity: { S: 'Малая', M: 'Средняя', L: 'Крупная' },
  introBody:
    'Карточки выполняете не вы — агенты. Поместите карточку в «Готово» с назначенным исполнителем, и агент возьмёт её в течение минуты. Без исполнителя задача не запустится. Сортировка: агент сначала переписывает идею в полноценную задачу. К выполнению: ожидает других карточек. По расписанию: ожидает таймера. «Выполняется» и «Проверка» — зоны агентов, не вмешивайтесь. Заблокировано: ждёт вас. Результаты появятся на карточке.',
  introGotIt: 'Понятно',
  evtCreated: (where, assignee) =>
    `создано${where ? ` в «${where}»` : ''}${assignee ? ` · назначено на ${assignee}` : ''}`,
  evtMovedTo: col => `перемещено в «${col}»`,
  evtParentReopened: parent => `родительская задача ${parent} снова открыта`,
  evtAssignedTo: assignee => `назначено на ${assignee}`,
  evtUnassigned: 'назначение снято',
  evtCommentBy: author => `комментарий от ${author}`,
  evtClaimedReview: 'взято агентом-проверяющим',
  evtClaimedWorker: 'взято воркером',
  evtWorkerStarted: 'воркер начал работу',
  evtCompleted: 'завершено',
  evtBlocked: 'заблокировано — требуется участие человека',
  evtUnblocked: col => `разблокировано${col ? ` → ${col}` : ' → Готово'}`,
  evtReclaimed: 'возвращено — снова в очереди',
  evtSpecified: 'спецификация написана агентом сортировки',
  evtPromoted: 'зависимости выполнены — переведено в «Готово»',
  evtScheduled: 'запланировано на потом',
  evtArchived: 'архивировано',
  evtReprioritized: priority => `приоритет изменён на ${priority}`,
  someone: 'кто-то',
  metaPriority: 'Приоритет',
  metaTenant: 'Арендатор',
  metaCreatedBy: 'Создано',
  metaCreated: 'Создано',
  metaWorkerPid: 'PID воркера',
  readyUnassignedTitle: 'Готово, но не назначено — эта карточка никогда не запустится.',
  readyUnassignedBody:
    'Диспетчер берёт в работу только карточки со статусом «Готово», у которых есть исполнитель. Выберите профиль в поле «Исполнитель» выше (или задайте исполнителя по умолчанию в настройках оркестрации) — и она запустится в течение минуты.',
  ...
  blockedBy: 'Заблокировано из-за',
  blocks: 'Блокирует',
  comments: n => `Комментарии · ${n}`,
  commentsHelpRunning:
    'Эта задача выполняется. Ваша заметка будет учтена воркером в текущем ходе в течение нескольких секунд — без блокировки и разблокировки. «Перезапустить с заметкой» вместо этого перезапускает задачу с нуля с вашей заметкой в контексте.',
  commentsHelp:
    'Комментарии добавляются в обсуждение задачи. Когда воркер берёт задачу, он читает их как часть контекста.',
  send: 'Отправить',
  comment: 'Комментарий',
  messageWorker: 'Сообщение работающему воркеру…',
  addComment: 'Добавить комментарий…',
  deliveredLive: 'Доставляется работающему воркеру в течение нескольких секунд.',
  requeueWithNote: 'Перезапустить с заметкой',
  notePosted: 'Заметка добавлена — воркер поставлен в очередь заново',
  activity: n => `Активность · ${n}`,
  runs: n => `Запуски · ${n}`,
  workerLog: 'Журнал воркера',
  workerLogTail: 'Журнал воркера · последние строки',
  ...
  boardImported: name => `Импортировано: ${name}`,
  boardImportedAs: slug => `Это имя уже занято — импортировано как ${slug}`,
  renameBoardTitle: 'Переименовать доску',
  deleteBoardTitle: name => `Удалить «${name}»?`,
  deleteBoardConfirm: 'Доска будет заархивирована, а не стёрта — её задачи и вложения останутся на диске, их можно восстановить.',
  ...
  slug: slug => `slug: ${slug}`,
  project: 'Проект',
  noProject: 'Без проекта (тестовые песочницы)',
  projectHintPre:
    'Новые задачи выполняются в репозитории проекта (отдельный worktree на задачу); каждая задача может переопределить своё рабочее пространство при создании. Управляйте проектами через ',
  projectHintCmd: 'hermes project',
  createBoard: 'Создать доску',
  orchestratorProfile: 'Профиль оркестратора',
  defaultAssignee: 'Исполнитель по умолчанию',
  defaultParen: '(по умолчанию)',
  autoDecompose: 'Автоматически декомпозировать задачи сортировки',
  profileDescriptions: 'Описания профилей',
  profileDescriptionsHint:
    'Описания направляют маршрутизацию декомпозера. Сгенерируйте автоматически вспомогательной моделью или напишите сами.',
  profileGoodAt: 'В чём хорош этот профиль?',
  auto: 'Авто',
  notify: {
    completedTitle: 'Задача завершена',
    blockedTitle: 'Задача заблокирована — требуется ваше участие',
    blockLoopTitle: 'Задача направлена в сортировку — требуется решение',
    gaveUpTitle: 'Задача отменена',
    crashedTitle: 'Воркер аварийно завершился — будет повторная попытка',
    timedOutTitle: 'Истекло время ожидания задачи — будет повторная попытка',
    openKanban: 'Открыть канбан',
    artifacts: (n: number) => `Файлов: ${n}`
  }
}
...
const ar: KanbanMessages = {
  plugin: {
    name: 'كانبان',
    description:
      'لوحة مهام لعدة وكلاء — صفحة للوحة، ومدخل في الشريط الجانبي، وعدد المهام الجارية في شريط الحالة.'
  },
  nav: 'كانبان',
  openBoard: 'كانبان: فتح اللوحة',
  newTaskCommand: 'كانبان: مهمة جديدة',
  countTip: (running, ready) => `كانبان — قيد التنفيذ ${running}، جاهزة ${ready}`,
  col: {
    triage: { label: 'الفرز', help: 'أفكار أولية — يتولى وكيل التحديد صياغتها إلى مواصفة.' },
    todo: { label: 'قيد الانتظار', help: 'بانتظار تبعيات، أو غير مُسندة.' },
    scheduled: { label: 'مجدولة', help: 'بانتظار حلول الوقت المحدد.' },
    ready: { label: 'جاهزة', help: 'التبعيات مكتملة — أسند ملفًا تعريفيًا وسيقوم الموزّع بتشغيلها.' },
    running: { label: 'قيد التنفيذ', help: 'تم استلامها من عامل — وكيل يعمل عليها الآن. يحددها الموزّع.' },
    blocked: { label: 'محظورة', help: 'طلب العامل تدخلًا بشريًا.' },
    review: { label: 'المراجعة', help: 'وكيل مراجعة يتحقق من العمل. يحدده الموزّع.' },
    done: { label: 'منجزة', help: 'اكتملت؛ المهام الفرعية التابعة تصبح جاهزة.' },
    archived: { label: 'مؤرشفة', help: 'مخفية من عرض اللوحة الافتراضي.' }
  },
  locked: {
    review: 'حالة المراجعة يحددها الموزّع عندما يستلم وكيل المراجعة البطاقة.',
    running: 'حالة قيد التنفيذ يحددها الموزّع عندما يستلم العامل البطاقة.',
    scheduled: 'حالة مجدولة تحتاج وقت تنبيه — يحدده الوكلاء؛ لا يمكن السحب إليها.'
  },
  arcRunning: 'يعمل وكيل على هذا الآن.',
  arcStale: 'تم استلامها، لكن لا نبض من العامل منذ دقيقتين أو أكثر — سيستعيدها الموزّع.',
  title: 'كانبان',
  orchestrationSettings: 'إعدادات التنسيق',
  newTask: 'مهمة جديدة',
  filterCards: 'تصفية البطاقات…',
  noMatch: 'لا توجد مهام مطابقة للمرشحات',
  noTasks: 'لا توجد مهام في هذه اللوحة',
  open: 'فتح',
  select: modifier => `تحديد (${modifier}+نقر)`,
  deselect: 'إلغاء التحديد',
  moveTo: label => `نقل إلى ${label}`,
  delete: 'حذف',
  reviewChecking: 'وكيل مراجعة يتحقق من العمل المكتمل.',
  attachedTip: name => `تم إسناد ${name} — سيسلّم الموزّع المهمة في الدورة التالية (≤ دقيقة واحدة).`,
  orchestratorTip: name => `سيستلم ${name} (المنسّق) هذه المهمة في الدورة التالية ويكتب المواصفة.`,
  autoAssignTip: name => `سيتم الإسناد تلقائيًا إلى "${name}" (kanban.default_assignee) في دورة الإرسال التالية.`,
  wontRun: 'لن تعمل',
  wontRunTip:
    'البطاقات الجاهزة لا تعمل إلا بعد إسناد ملف تعريفي. افتح البطاقة وحدد مسؤولًا، أو اضبط مسؤولًا افتراضيًا في إعدادات التنسيق.',
  noHeartbeat: 'لا نبض',
  expand: label => `توسيع ${label}`,
  collapse: label => `طي ${label}`,
  newTaskIn: label => `مهمة جديدة في ${label}`,
  empty: 'فارغة',
  unassigned: 'غير مُسندة',
  filters: 'المرشحات',
  allProfiles: 'كل الملفات التعريفية',
  allTenants: 'كل المستأجرين',
  showArchived: 'إظهار المؤرشف',
  groupRunning: 'تجميع قيد التنفيذ حسب الملف التعريفي',
  nSelected: n => `تم تحديد ${n}`,
  moveToShort: 'نقل إلى',
  assign: 'إسناد',
  ...
  refused: 'مرفوضة',
  bulkFailed: (failed, total, err) => `فشل ${failed} من أصل ${total} — ${err}. تبقى البطاقات الفاشلة محددة.`,
  titlePlaceholderTriage: 'فكرة أولية — سيقوم وكيل التحديد بصياغتها',
  ...
  model: 'النموذج',
  modelInherit: 'افتراضي الملف التعريفي',
  modelClear: 'إزالة تجاوز النموذج',
  modelHint: 'يشغّل هذه المهمة على نموذج وعمق تفكير محددين. عدم التحديد يستخدم إعداد الملف التعريفي المُسند.',
  workspaceInherit: 'يرث دليل مشروع اللوحة',
  workspaceInheritDir: dir => `اتركه فارغًا للوراثة من ${dir}`,
  workspaceInheritGeneric: 'اتركه فارغًا لوراثة دليل مشروع اللوحة.',
  assignee: 'المسؤول',
  defaultOption: name => `${name} (افتراضي)`,
  parkedOption: 'غير مُسندة (معلّقة — لن تعمل)',
  skills: 'المهارات (مفصولة بفواصل)',
  skillsPlaceholder: 'translation, github',
  parent: 'المهمة الأصل (تحظر حتى اكتمالها)',
  noParent: '— بلا مهمة أصل —',
  goalMode: 'وضع الهدف (يستمر العامل بالتكرار حتى يوافق الحكم على الاكتمال)',
  ...
  makesModelCall: 'يستدعي النموذج',
  estimateTip: 'تقدير تقريبي للرموز والتعقيد من النموذج المساعد — يستدعي النموذج.',
  estimateTipLong: 'يجري استدعاءً سريعًا للنموذج المساعد لتقدير الرموز والتعقيد. دليل تقريبي وليس فاتورة.',
  roughEstimate: 'تقدير تقريبي',
  tokUnit: 'رمز',
  couldNotEstimate: 'تعذّر التقدير',
  complexity: { S: 'صغيرة', M: 'متوسطة', L: 'كبيرة' },
  introBody:
    'أنت لا تشغّل البطاقات — الوكلاء يفعلون ذلك. ضع بطاقة في "جاهزة" مع مسؤول وسيلتقطها وكيل خلال دقيقة. بلا مسؤول، لا تعمل. الفرز: يعيد وكيل صياغة الفكرة كمهمة صحيحة أولًا. قيد الانتظار: بانتظار بطاقات أخرى. مجدولة: بانتظار مؤقت. قيد التنفيذ والمراجعة: مسارا الوكلاء، لا تتدخل. محظورة: بانتظارك أنت. تعود النتائج إلى البطاقة.',
  introGotIt: 'فهمت',
  evtCreated: (where, assignee) =>
    `أُنشئت${where ? ` في ${where}` : ''}${assignee ? ` · أُسندت إلى ${assignee}` : ''}`,
  evtMovedTo: col => `نُقلت إلى ${col}`,
  evtParentReopened: parent => `أُعيد فتح المهمة الأصل ${parent}`,
  ...
  evtWorkerStarted: 'بدأ العامل العمل',
  evtCompleted: 'اكتملت',
  evtBlocked: 'محظورة — تحتاج تدخلًا بشريًا',
  evtUnblocked: col => `أُلغي الحظر${col ? ` → ${col}` : ' → جاهزة'}`,
  evtReclaimed: 'استُعيدت — أُعيدت إلى قائمة الانتظار',
  evtSpecified: 'كتب وكيل الفرز المواصفة',
  evtPromoted: 'اكتملت التبعيات — تمت الترقية إلى جاهزة',
  evtScheduled: 'جُدولت لوقت لاحق',
  evtArchived: 'أُرشفت',
  evtReprioritized: priority => `تم ضبط الأولوية إلى ${priority}`,
  someone: 'أحدهم',
  metaPriority: 'الأولوية',
  metaTenant: 'المستأجر',
  metaCreatedBy: 'أنشأها',
  metaCreated: 'تاريخ الإنشاء',
  metaWorkerPid: 'معرف عملية العامل',
  readyUnassignedTitle: 'جاهزة لكن غير مُسندة — لن تعمل هذه البطاقة أبدًا.',
  readyUnassignedBody:
    'يلتقط الموزّع فقط البطاقات الجاهزة التي لها مسؤول. اختر ملفًا تعريفيًا في حقل المسؤول أعلاه (أو اضبط مسؤولًا افتراضيًا في إعدادات التنسيق) وستعمل خلال دقيقة.',
  ...
  blockedBy: 'محظورة بسبب',
  blocks: 'تحظر',
  comments: n => `التعليقات · ${n}`,
  commentsHelpRunning:
    'هذه المهمة قيد التنفيذ. تُدمج ملاحظتك في الدورة الحالية للعامل خلال ثوانٍ — دون الحاجة لحظر أو إلغاء حظر. "إعادة الطابور مع ملاحظة" تعيد تشغيل المهمة من البداية مع وضع ملاحظتك في السياق بدلًا من ذلك.',
  commentsHelp:
    'تُضاف التعليقات إلى سلسلة المهمة. عندما يلتقط العامل المهمة يقرأها كجزء من سياقه.',
  send: 'إرسال',
  comment: 'تعليق',
  messageWorker: 'مراسلة العامل قيد التشغيل…',
  addComment: 'إضافة تعليق…',
  deliveredLive: 'تُسلَّم إلى العامل قيد التشغيل خلال ثوانٍ.',
  requeueWithNote: 'إعادة الطابور مع ملاحظة',
  notePosted: 'تم نشر الملاحظة — أُعيد العامل إلى الطابور',
  activity: n => `النشاط · ${n}`,
  runs: n => `التشغيلات · ${n}`,
  ...
  copiedId: id => `تم نسخ ${id}`,
  copiedTitle: 'تم نسخ العنوان',
  close: 'إغلاق',
  working: 'قيد العمل',
  board: 'اللوحة',
  newBoard: 'لوحة جديدة',
  ...
  settingsDots: 'إعدادات…',
  exportBoardTitle: 'تصدير اللوحة…',
  importBoardTitle: 'استيراد اللوحة…',
  boardExported: path => `تم تصدير اللوحة إلى ${path}`,
  boardImported: name => `تم استيراد ${name}`,
  boardImportedAs: slug => `هذا الاسم مستخدم بالفعل — تم الاستيراد باسم ${slug}`,
  ...
  boardArchived: path => `تمت أرشفة اللوحة إلى ${path}`,
  boardSettingsFor: name => `إعدادات اللوحة — ${name}`,
  name: 'الاسم',
  boardNamePlaceholder: 'اسم اللوحة',
  slug: slug => `المعرف النصي: ${slug}`,
  project: 'المشروع',
  noProject: 'بلا مشروع (بيئات تجريبية)',
  projectHintPre:
    'تعمل المهام الجديدة في مستودع المشروع (شجرة عمل منفصلة لكل مهمة)؛ يمكن لكل مهمة تجاوز مساحة عملها عند الإنشاء. إدارة المشاريع عبر ',
  projectHintCmd: 'hermes project',
  createBoard: 'إنشاء لوحة',
  orchestratorProfile: 'ملف تعريف المنسّق',
  defaultAssignee: 'المسؤول الافتراضي',
  defaultParen: '(افتراضي)',
  autoDecompose: 'تفكيك مهام الفرز تلقائيًا',
  profileDescriptions: 'أوصاف الملفات التعريفية',
  profileDescriptionsHint:
    'توجّه الأوصاف توجيه المُفكِّك. أنشئها تلقائيًا بالنموذج المساعد، أو اكتبها بنفسك.',
  profileGoodAt: 'ما الذي يجيده هذا الملف التعريفي؟',
  auto: 'تلقائي',
  notify: {
    completedTitle: 'اكتملت المهمة',
    blockedTitle: 'المهمة محظورة — تحتاج مدخلاتك',
    blockLoopTitle: 'أُحيلت المهمة إلى الفرز — تحتاج قرارًا',
    gaveUpTitle: 'تم التخلي عن المهمة',
    crashedTitle: 'تعطّل العامل — ستتم إعادة المحاولة',
    timedOutTitle: 'انتهت مهلة المهمة — ستتم إعادة المحاولة',
    openKanban: 'فتح كانبان',
    artifacts: (n: number) => `${n} من الملفات الناتجة`
  }
}

export const KANBAN_LOCALES: PluginLocaleBundles = { en, ja, zh, 'zh-hant': zhHant, de, ru, ar }
